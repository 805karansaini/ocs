...
class ScreenUserInputs(object):
    ...
    def create_user_inputs_tab(self):
        ...
        user_inputs_table_columns = ['Leg Description','Action',  'Delta Range Min', 'Delta Range Max']
        ...
        for column_name in user_inputs_table_columns:
            ...
            if column_name in ['Leg Description']:
                width = 900

            if column_name in ['Action']:

                width = 0

            self.user_inputs_table.column(column_name, anchor=tk.W, width=width, stretch='no')
        ...
        for column_name in user_inputs_table_columns:
            self.user_inputs_table.heading(column_name, text=column_name, anchor="center")
        ...
        # create combo box
        combo_box = tkinter.ttk.Combobox(user_inputs_table_frame, state="readonly",
        ...
        def on_double_click(event):
            ...
            if user_input_name in entry_list:
                ...
                entry.insert(0, self.user_inputs_table.item(item, 'values')[int(column[1:]) - 1])
                ...
                if user_input_name in true_or_false_inputs:
                    ...
                    # Clear the current options and set new ones
                    combo_box['values'] = limit_price_for_execution_engine_parameter_list
    ...
    def update_uer_inputs_table(self):
        ...
        user_inputs_description_list = ['IPv4 address for connecting TWS.',
        ...
        # create df
        user_inputs_df = pd.DataFrame(columns=['User Input Name', 'User Input Value', 'User Input Description'])


        # map user input to its description
        map_user_input_to_description = dict(zip(user_inputs_name_list, user_inputs_description_list))

        map_user_input_to_value = dict(zip(user_inputs_name_list, user_inputs_value_list))


        # User input grouping dictionary
        user_input_grouping_map_dict = {'IBKR Inputs':{
                        'System Flags':[
                            'ibkr_tws_host',
                            'ibkr_tws_port',
                            'ibkr_tws_connection_id',
        ...
        user_inputs_name_list, user_inputs_value_list, user_inputs_description_list = self.group_user_inputs(user_input_grouping_map_dict, map_user_input_to_description, map_user_input_to_value)
        ...
        tag_name = f'bold'
        self.user_inputs_table.tag_configure(tag_name, font=('TkDefaultFont', 10, 'bold'))
