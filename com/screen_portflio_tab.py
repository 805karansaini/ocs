...
class ScreenPortfolio(object):
    ...
    def get_temp_lots_entry_price_list(
        self, temp_lots, temp_entry_price_list, temp_lots_list, current_lots
    ):
        # Init
    ...
    def update_temp_lots_entry_price_list(
        self, temp_lots, temp_entry_price_list, temp_lots_list
    ):
        # Init
    ...
    def create_portfolio_tab(self):
        ...
        def on_combobox_select(event):
            selected_item = (
                self.zero_position_options_combo_box.get()
        ...
        self.zero_position_options_combo_box = ttk.Combobox(
            buttons_frame,
            width=18,
            values=zero_position_options_list,
            state="readonly",
            style="Custom.TCombobox",
        )
        ...
        self.zero_position_options_combo_box.bind(
        ...
        # Create headings for portfolio table
        for column_name in portfolio_table_columns:
            self.portfolio_legs_table.heading(
    ...
    def display_portfolio_combo_gui_pop_up(self):
        ...
        def add_portfolio_combo():
            ...
            local_map_con_id_to_contract = copy.deepcopy(
            ...
            for indx, row in local_legs_df.iterrows():
                ...
                leg_values_list = [
                    ...
                    leg_obj.multiplier,
                    leg_obj.exchange,
                    leg_obj.tradingClass,
                    leg_obj.currency,
                    leg_obj.conId,
                    leg_obj.primaryExchange,
                    leg_obj.strike,
                    leg_obj.lastTradeDateOrContractMonth,
                ]

                list_of_tuple_of_values.append(tuple(leg_values_list))

            # Create combo obj
            combo_obj, unique_id_added = create_combo_wrapper(
                list_of_tuple_of_values,
                0,
                False,
                False,
        ...
        try:
            ...
            for indx, row in local_position_table.iterrows():
                ...
                if float(net_position) > 0:
                    ...
                    for entry_price, lots, action in zip(
                        entry_price_list, lots_list, action_list
                    ):
                        if (
                            action == "BUY"
                            and lots.isnumeric()
                            and is_float(entry_price)
                        ):
                    ...
                    if flag_first:
                        ...
                        if temp_entry_price_list != []:
                            ...
                            temp_price = np.sum(
                                ...
                                * np.array(temp_lots_list)
                            ) / np.sum(np.array(temp_lots_list))
                        ...
                        if temp_lots > 0 and lots < 0 and abs(temp_lots) > abs(lots):
                            ...
                            new_temp_price_list, new_lots_list = (
                                self.get_temp_lots_entry_price_list(
                                    temp_lots,
                                    temp_entry_price_list,
                                    temp_lots_list,
                                    lots,
                            ...
                            # Calculating weighted average
                            temp_price = np.sum(
                                np.array(new_temp_price_list) * np.array(new_lots_list)
                            ) / np.sum(np.array(new_lots_list))

                            realized_pnl += abs(lots) * (
                                float(entry_price) - temp_price
                            )
                            ...
                            temp_entry_price_list, temp_lots_list = (
                                self.update_temp_lots_entry_price_list(
                                    temp_lots, temp_entry_price_list, temp_lots_list
                                )
                            )
                ...
                if entry_price_process_list != []:
                    ...
                    if entry_price_list != []:
                        ...
                        weighted_average = round(
                            np.sum(np.array(entry_price_list) * np.array(lots_list))
                            / np.sum(np.array(lots_list)),
                            2,
                        )
                    ...
                    if flag_first:
                        ...
                        if temp_entry_price_list != []:
                            ...
                            temp_price = round(
                                ...
                                / np.sum(np.array(temp_entry_price_list)),
                                2,
                            )
    ...
    def update_portfolio_combo_table(self, flag_delete=False):
        ...
        local_position_table["Table ID"] = (
            local_position_table["Unique ID"].astype(str)
            + "_"
            + local_position_table["Account ID"]
        )

        local_position_table = local_position_table[
            variables.portfolio_table_combo_columns + ["Table ID"]
        ]

        # get entry price, realized pn and unrealized pnl
        list_entry_price, unreal_pnl_list, realized_pnl_list = (
            self.calculate_pnl_entry_price(local_position_table)
        )
        ...
        try:
            local_position_table["#Net Position"] = local_position_table[
                "#Net Position"
            ].astype(int)

            local_position_table["Realized PNL"] = local_position_table[
                "Realized PNL"
            ].astype(float)

            local_position_table["Unrealized PNL"] = local_position_table[
                "Unrealized PNL"
            ].astype(float)

            local_position_table["Entry Price"] = local_position_table[
                "Entry Price"
            ].astype(float)
            ...
            new_row = [
                ...
                "None",
                "Total",
                local_position_table["#Net Position"].sum(),
                local_position_table["Realized PNL"].sum(),
                local_position_table["Unrealized PNL"].sum(),
                "None",
                local_position_table["Entry Price"].sum(),
                "-1",
            ]
            ...
            local_position_table = pd.concat(
                [local_position_table, new_row_df], ignore_index=True
            )
            ...
            for indx, row in local_position_table.iterrows():
                ...
                if (
                    "ALL" in variables.account_ids_list_of_selected_acount_group
                    or account_id in variables.account_ids_list_of_selected_acount_group
                ):
                    if zero_position_flag in ["No"] and net_position == 0:
                        continue
    ...
    def update_portfolio_legs_table(self, flag_delete=False):
        ...
        for unique_id in local_unique_id_to_combo_obj:
            ...
            for leg_obj in all_legs:
                ...
                if leg_obj.con_id not in legs_decription_dict:
                    # add legs details
                    legs_decription_dict[leg_obj.con_id] = {
                        key: self.make_informative_leg_string(leg_obj)
                        for key, value in local_position_dict[int(unique_id)].items()
                    }

                    legs_decription_list += [
                        self.make_informative_leg_string(leg_obj)
                        for key, value in local_position_dict[int(unique_id)].items()
                    ]

                    legs_positions_dict[leg_obj.con_id] = {
                        key: self.calculate_legs_position(leg_obj.con_id, key)
                        for key, value in local_position_dict[int(unique_id)].items()
                    }

                    legs_positions_list += [
                        value * int(leg_obj.quantity)
                        for key, value in local_position_dict[int(unique_id)].items()
                    ]
        ...
        legs_positions_list = [
            value
            for inner_dict in legs_positions_dict.values()
        ...
        entry_list, unrealize_pnl, realize_pnl = (
            self.calculate_pnl_entry_price_for_legs_table(self.legs_df)
        )
        ...
        try:
            ...
            self.legs_df["Unrealized PNL"] = self.legs_df["Unrealized PNL"].astype(
                float
            )
            ...
            new_row = [
                "Total",
                self.legs_df["#Net Position"].sum(),
                "None",
                round(self.legs_df["Realized PNL"].sum(), 2),
                round(self.legs_df["Unrealized PNL"].sum(), 2),
                round(self.legs_df["Entry Price"].sum(), 2),
                "-1",
                "None",
            ]
            ...
            self.legs_df["Realized PNL"] = self.legs_df["Realized PNL"].map(
                lambda x: f"{x:,.2f}"
            )
            self.legs_df["Unrealized PNL"] = self.legs_df["Unrealized PNL"].map(
                lambda x: f"{x:,.2f}"
            )
            self.legs_df["Entry Price"] = self.legs_df["Entry Price"].map(
                lambda x: f"{x:,.2f}"
            )
