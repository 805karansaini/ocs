...
class ChangeUnderlyingOptionsPrice:
        ...
        for (
            ...
            if underlying_sec_type == "FUT":
                (
                    ...
                    closest_expiry_date,
                    underlying_conid,
                ) = ChangeUnderlyingOptionsPrice.get_strike_and_closet_expiry_for_fop(
                ...
                (
                    all_strikes,
                    closest_expiry_date,
                ) = ChangeUnderlyingOptionsPrice.get_strike_and_closet_expiry_for_opt(
                    symbol,
                    dte,
                    underlying_sec_type,
                    exchange,
            ...
            list_of_req_ids_for_volume = (
                ChangeUnderlyingOptionsPrice.fetch_historical_data(
                    [underlying_contract],
                    StrategyVariables.bar_size_chg_underlying_price,
                    StrategyVariables.lookback_days_chg_underlying_price,
                )
            )
            ...
            for req_id in list_of_req_ids_for_volume:
                ...
                if not df.empty:
                    ...
                    for date, timestamp in earliest_timestamps.items():
                        if date == latest_date:
                            open_price_today_morning = df_sorted.loc[
                                df_sorted["Time"] == timestamp, "Close"
                            ].iloc[0]
                            break
                    ...
                    for date, timestamp in earliest_timestamps.items():
                        if date == date_14_days_ago:
                            open_price_14_days_ago_morning = df_sorted.loc[
                                df_sorted["Time"] == timestamp, "Close"
                            ].iloc[0]
                            break
                else:
                ...
                for item in data:
                    ...
                    if delta is not None:
                        for target_delta in target_deltas:
                            ...
                            if closest_strikes[target_delta] is None or abs(
                                delta - target_delta
                            ) < abs(closest_strikes[target_delta] - target_delta):
            ...
            for right in list_of_rights:
                ...
                for contract, (
                    ...
                    call_oi,
                    put_oi,
                ) in zip(list_of_filtered_option_contracts, bid_ask_tuple):
                ...
                for req_id, contract in zip(
                    list_of_req_ids_for_volume, list_of_filtered_option_contracts
                ):
                    ...
                    if not df.empty:
                        ...
                        for date, timestamp in earliest_timestamps.items():
                            if date == latest_date:
                                open_option_price_today_morning = df_sorted.loc[
                                    df_sorted["Time"] == timestamp, "Close"
                                ].iloc[0]
                                break
            ...
            for right in list_of_rights:
                ...
                for contract, (
                    ...
                    if all([ask_price, bid_price]):
                        ...
                        time_to_expiration = (
                            abs(expiry_date_obj - current_date_obj).days + 14
                        ) / 365
                ...
                closest_strikes_14d = {
                    target_delta: None for target_delta in target_deltas
                }
                ...
                for item in data_for_all_contract_to_get_nearest_strike:
                    ...
                    if delta is not None:
                        for target_delta in target_deltas:
                            # If closest strike is not set or if current strike is closer to target delta
                            if closest_strikes_14d[target_delta] is None or abs(
                                delta - target_delta
                            ) < abs(closest_strikes_14d[target_delta] - target_delta):
            ...
            for right in list_of_rights:
                ...
                for req_id, contract in zip(
                    list_of_req_ids_for_volume, list_of_filtered_option_contracts_14d
                ):
                    ...
                    if not df.empty:
                        ...
                        for date, timestamp in earliest_timestamps.items():
                            if date == date_14_days_ago:
                                close_option_price_14_days_ago_morning = df_sorted.loc[
                                    df_sorted["Time"] == timestamp, "Close"
                                ].iloc[0]
                                break
            ...
            if open_price_today_morning and current_price_today:
                ...
                change_underlying_price = (
                    (current_price_today - open_price_today_morning)
                    / open_price_today_morning
                ) * 100
            if open_price_14_days_ago_morning and current_price_today:
                """Change in Underlying 14days"""
                change_underlying_price_14d = (
                    current_price_today - open_price_14_days_ago_morning
                )
            ...
            if current_option_price and open_option_price_today_morning:
                change_option_price = (
                    (current_option_price - open_option_price_today_morning)
                    / open_option_price_today_morning
                ) * 100
                if change_option_price != 0:
                    Change_underlying_options_price_today = (
                        change_underlying_price / change_option_price
                    )
                else:
            ...
            if (
                ...
                and close_option_price_14_days_ago_morning
                and change_underlying_price_14d
            ):
                change_option_price_14d = (
                    current_option_price - close_option_price_14_days_ago_morning
                )
                if change_option_price_14d != 0:
                    chg_uderlying_opt_price_14d = (
                        change_underlying_price_14d / change_option_price_14d
                    )
                else:
            ...
            ChangeUnderlyingOptionsPrice.update_hv_calculation(
                indicator_id,
                Change_underlying_options_price_today,
                chg_uderlying_opt_price_14d,
            )
    ...
    def update_hv_calculation(
        indicator_id, Change_underlying_options_price_today, chg_uderlying_opt_price_14d
    ):

        values_dict = {
            "Change_underlying_options_price_today": Change_underlying_options_price_today,
        ...
        ChangeUnderlyingOptionsPrice.scanner_indicator_tab_chgopt_obj.update_into_indicator_table(
