...
class HistoricalVolatility:
        ...
        if True or variables.flag_enable_hv_annualized:
            if historical_volatility_value not in ["N/A", None]:
                ...
                annualized_historical_volatility_value = (
                    historical_volatility_value * math.sqrt(255)
                )
    ...
    def update_hv_calculation(conid, map_conid_to_list_of_indicators_id, values_dict):
        ...
        for indicator_id in list_of_indicator_id:
            ...
            IndicatorHelper.update_indicator_values_for_indcator_id(
                indicator_id=indicator_id, values_dict=values_dict
            )
    ...
    def compute():
        ...
        for con_id, req_id in map_conid_to_req_id.items():
            ...
            for indx in range(
                StrategyVariables.user_input_average_historical_volatility_days
            ):
                # Start and End Date
                ...
                try:
                    ...
                    if filtered_df.empty:
                        ...
                        if StrategyVariables.flag_store_csv_files:
                            ...
                            file_name = rf"{StrategyVariables.user_input_average_historical_volatility_days - indx}_{start_date}_to_{end_date}"
                            StrategyUtils.save_option_combo_scanner_csv_file(
                                filtered_df, folder_name, file_name
                            )
                    ...
                    if StrategyVariables.flag_test_print:
                        print(
                            f"Day {indx} {start_date} to {end_date} HV: {hv_value}, having con_id: {con_id}"
                        )
                except Exception as e:
                    pass
            ...
            current_underlying_hv_value = (
                list_of_hv_values_for_n_days[-1]
                if len(list_of_hv_values_for_n_days) > 0
                else None
            )
            if StrategyVariables.flag_test_print:
                print(
                    f"Current HV value for conid: {con_id},",
                    current_underlying_hv_value,
                )

            # Average HV over N-Days
            if len(list_of_hv_values_for_n_days) != 0:
                average_underlying_hv_over_n_days = sum(
                    list_of_hv_values_for_n_days
                ) / len(list_of_hv_values_for_n_days)
            else:
            ...
            if StrategyVariables.flag_test_print:
                ...
                print(
                    f"Conid: {con_id} OldDates: One Day: {one_day_ago_date} N Day: {n_day_ago_date}"
                )
            ...
            try:
                absoulte_change_in_underlying_over_one_day = (
                    current_close_price - one_day_ago_close_price
                )
            ...
            try:
                absoulte_change_in_underlying_over_n_days = (
                    current_close_price - n_day_ago_close_price
                )
                ...
                percentage_change_in_underlying_over_n_days = (
                    (current_close_price - n_day_ago_close_price)
                    / n_day_ago_close_price
                ) * 100
            except Exception as e:
                print(
                    f"Exception Absolte chg in underlying ove n days for conid {con_id} e: {e}"
                )
            ...
            values_dict = {
                ...
                "current_underlying_hv_value": (
                    round(current_underlying_hv_value, 2)
                    if current_underlying_hv_value
                    else current_underlying_hv_value
                ),
                "average_underlying_hv_over_n_days": (
                    round(average_underlying_hv_over_n_days, 2)
                    if average_underlying_hv_over_n_days
                    else average_underlying_hv_over_n_days
                ),
            }
            ...
            HistoricalVolatility.update_hv_calculation(
                con_id, map_conid_to_list_of_indicators_id, values_dict
            )
