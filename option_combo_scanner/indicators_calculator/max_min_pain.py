...
class Max_Min_Pain:
    ...
    def compute():
        ...
        for (
            ...
            if underlying_sec_type == "FUT":
                all_strikes, closest_expiry_date, underlying_conid = Max_Min_Pain.get_strike_and_closet_expiry_for_fop(
                ...
                all_strikes, closest_expiry_date = Max_Min_Pain.get_strike_and_closet_expiry_for_opt(
                    symbol,
                    dte,
                    underlying_sec_type,
                    exchange,
    ...
    def calculate_max_pain(strike_oi_df):
        ...
        for i in range(1, days):
            call_loss[i] = sum(strike_oi_df['call_oi'][j] * (strike_oi_df['Strike'][i] - strike_oi_df['Strike'][j]) for j in range(i))

        # Calculate put losses
        for i in range(days - 2, -1, -1):
            put_loss[i] = sum(strike_oi_df['put_oi'][j] * (strike_oi_df['Strike'][j] - strike_oi_df['Strike'][i]) for j in range(days - 1, i, -1))
    ...
    def update_values_in_db_gui(min_pain, max_pain, indicator_id):
        ...
        if indicator_id in StrategyVariables.map_indicator_id_to_indicator_object:
            StrategyVariables.map_indicator_id_to_indicator_object[indicator_id].min_pain = min_pain
            StrategyVariables.map_indicator_id_to_indicator_object[indicator_id].max_pain = max_pain
        ...
        Max_Min_Pain.scanner_indicator_tab_pain_obj.update_into_indicator_table(StrategyVariables.scanner_indicator_table_df)
