...
class PutCallVol:
    ...
    def create_daily_candle_df_from_raw_df(df):
        ...
        try:
            ...
            print(f"Inside PutCallVol: create_daily_candle_df_from_raw_df Exception {e}")
    ...
    def get_total_vol_for_date(date, call_list_of_daily_volume_data_df, put_list_of_daily_volume_data_df):
        ...
        for call_df_for_strike_s in call_list_of_daily_volume_data_df:
            ...
            if not call_df_for_strike_s.empty:
                vol_for_date = call_df_for_strike_s[call_df_for_strike_s["Date"] == date]["Volume"].sum()
        ...
        for put_df_for_strike_s in put_list_of_daily_volume_data_df:
            if not put_df_for_strike_s.empty:
                vol_for_date = put_df_for_strike_s[put_df_for_strike_s["Date"] == date]["Volume"].sum()
    ...
    def compute(
            ...
            for req_id, contract in zip(list_of_req_id_for_historical_data, list_of_contracts):
                ...
                if right == "Call":
                    ...
                    if StrategyVariables.flag_store_csv_files:
                        folder_name = fr"PutCallVol\{indicator_object.indicator_id}\Call"
                        file_name = fr"Call_{contract.strike}"
                    ...
                    if StrategyVariables.flag_store_csv_files:
                        folder_name = fr"PutCallVol\{indicator_object.indicator_id}\Put"
        ...
        if list_of_daily_volume_data_df_for_call and list_of_daily_volume_data_df_for_put:
            list_of_daily_volume_data_df_of_combined_call_and_put = (
                list_of_daily_volume_data_df_for_call + list_of_daily_volume_data_df_for_put
            )
            all_dates = pd.concat([df["Date"] for df in list_of_daily_volume_data_df_of_combined_call_and_put if not df.empty])
        else:
            return
        ...
        if len(sorted_dates_set) > 1:
            ...
            put_call_vol_ratio_yesterday = map_date_to_put_call_volume_ratio[yesterday_date]

            if put_call_volume_ratio_current_day and put_call_vol_ratio_yesterday:
                absolute_pc_change_since_yesterday = put_call_volume_ratio_current_day - put_call_vol_ratio_yesterday
            else:
        ...
        values_dict = {
            "put_call_volume_ratio_current_day": (
                round(put_call_volume_ratio_current_day, 2) if put_call_volume_ratio_current_day else put_call_volume_ratio_current_day
            ),
            ...
            "absolute_pc_change_since_yesterday": (
                round(absolute_pc_change_since_yesterday, 2) if absolute_pc_change_since_yesterday else absolute_pc_change_since_yesterday
            ),
        }
