...
#         self.local_config_object = copy.deepcopy(StrategyVariables.config_object)
...
#         for leg_object in self.local_config_object['list_of_config_leg_object']:
#             print(leg_object)
...
def insert_combinations_into_db(
        ...
        config_obj = self.config_obj
        list_of_config_leg_object = config_obj.list_of_config_leg_object
        instrument_ids = [leg.instrument_id for leg in list_of_config_leg_object]
        instrument_details = [StrategyVariables.map_instrument_id_to_instrument_object[leg.instrument_id] for leg in list_of_config_leg_object]
        # Get all the combo_id for this very instrument and expiry pair(config_id)
        where_condition = (
        ...
        values_dict = {          
            "instrument_id": ','.join(map(str, instrument_ids)),
            "config_id": config_obj.config_id,
            "number_of_legs": config_obj.no_of_leg,
            "symbol": [instrument.symbol for instrument in instrument_details],
            "sec_type": [instrument.sec_type for instrument in instrument_details],
            # "expiry": expiry,
            "right": [config_obj.right] * len(instrument_ids),
            "multiplier": [instrument.multiplier for instrument in instrument_details],
            "trading_class": [instrument.trading_class for instrument in instrument_details],
            "currency": [instrument.currency for instrument in instrument_details],
            "exchange": [instrument.exchange for instrument in instrument_details],
            # "max_profit_max_loss_ratio": 1.3,
            "primary_exchange": [instrument.primary_exchange for instrument in instrument_details],
        }

        for combination, combo_net_delta in zip(list_of_all_generated_combination, list_of_combo_net_deltas):
            expiry_dates = [combo[0][-1] for combo in combination]
            # print(f"\n Symbol: {instrument_object.symbol} Combination: {combination}")
            ...
            # add combo net delta calculation to the value dict
            values_dict["combo_net_delta"] = combo_net_delta
            values_dict['expiry'] = expiry_dates
            # values_dict["max_profit"] = "inf" if max_profit == float("inf") else max_profit
            # values_dict["max_loss"] = "-inf" if max_loss == float("-inf") else max_loss
            res, combo_id = SqlQueries.insert_into_db_table(table_name="combination_table", values_dict=values_dict)
            if not res:
            ...
            # list of leg object
            list_of_all_leg_objects = []
            for index, ((strike, delta, con_id, expiry), config_leg_object) in enumerate(zip(combination, list_of_config_leg_object)):
                leg_values_dict = {
                    "combo_id": combo_id,
                    "leg_number": index + 1,
                    "con_id": con_id,
