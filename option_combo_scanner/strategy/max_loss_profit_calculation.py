...
class MaxPNLCalculation:
    ...
    def create_group_same_und(combination, flag_return_dict=False):
        ...
        for (
            ...
            if underlying_conid in grouped_tuples:
                grouped_tuples[underlying_conid].append(
                    (
                        symbol,
                        strike,
                        delta,
            ...
            # Create the group key: list and append the leg
                ...
                grouped_tuples[underlying_conid] = [
                    (
                        symbol,
                        strike,
                        delta,
    ...
    def calcluate_max_pnl(combination, list_of_config_leg_object):
        ...
        flag_overall_nearest_expiry = (
            strategy_variables.calculation_mode_for_combination_max_pnl
        )
        ...
        for leg_tuple, config_leg_obj in zip(
            list_of_leg_tuples, list_of_config_leg_object
        ):
        ...
        list_of_combination_groups: list = MaxPNLCalculation.create_group_same_und(
            list_of_leg_tuples_with_config_leg
        )

        # Get the list of closest expiry for the groups
        list_of_closest_expiry_for_each_group = (
            MaxPNLCalculation.find_closest_expiry_for_groups(list_of_combination_groups)
        )
        ...
        if flag_overall_nearest_expiry == MaxPNLEnum.ONE:
            ...
            list_of_closest_expiry_for_each_group = [cloest_expiry_] * len(
                list_of_closest_expiry_for_each_group
            )
        else:
        ...
        for combination_group, closest_expiry in zip(
            list_of_combination_groups, list_of_closest_expiry_for_each_group
        ):
            # Creating a list of the config_leg_object for this group, for getting action, right etc info
            list_of_config_leg_object_for_combination_group = [
                _[-1] for _ in combination_group
            ]
            ...
            max_loss_combination_group, max_profit_combination_group = (
                MaxPNLCalculation.get_combination_max_loss_and_max_profit(
    ...
    def get_combination_max_loss_and_max_profit(
        list_of_legs_tuple, list_of_config_leg_objects, closest_expiry
    ):
        """
        ...
        combination_premium_received = MaxPNLCalculation.calculate_combination_premium(
            list_of_legs_tuple,
            list_of_config_leg_objects,
        )
        ...
        if (
            ...
            print(
                f"Inside get_combination_max_loss_and_max_profit function could not find instrument id: {instrument_id}"
            )
            return None, None
        instrument_object_for_multiplier = copy.deepcopy(
            strategy_variables.map_instrument_id_to_instrument_object[instrument_id]
        )
        ...
        for i, leg_tuple in enumerate(list_of_legs_tuple):
            ...
            if i == 0:
                ...
                list_of_combination_payoff_at_all_strikes.append(
                    round(strike_zero_payoff, 2)
                )
                ...
                slope_left_numerator = (strike_zero_payoff - strike_one_payoff) / int(
                    instrument_object_for_multiplier.multiplier
                )
                slope_left_deno = 1
            ...
            list_of_combination_payoff_at_all_strikes.append(
                round(combo_pay_off_for_current_strike, 2)
            )
            ...
            if i == len(list_of_legs_tuple) - 1:
                ...
                list_of_combination_payoff_at_all_strikes.append(
                    round(max_strike_right_payoff, 2)
                )
                ...
                slope_right_numerator = (
                    max_strike_right_payoff - max_strike_right_one_payoff
                ) / int(instrument_object_for_multiplier.multiplier)
                slope_right_deno = 1
        ...
        list_of_combination_payoff_at_all_strikes = [
            _ + combination_premium_received
            for _ in list_of_combination_payoff_at_all_strikes
        ]
    ...
    def get_combination_payoff(
        ...
        for leg, config_leg_obj in zip(list_of_legs_tuple, list_of_config_leg_object):
            ...
            if (
                ...
                print(
                    f"Inside get_combination_payoff function could not find instrument id: {instrument_id}"
                )
                return 0
            else:
                if multiplier == None:
            ...
            # Calculate the OptionPayoff for the further expiry than the closest expiry, holds the TimeValueOfOption
                ...
                closest_expiry_obj = datetime.datetime.strptime(
                    closest_expiry, "%Y%m%d"
                )
                ...
                # Theoretical Premium
                leg_payoff = Utils.get_theoretical_premium(
                    underlying_strike_price,
                    strategy_variables.riskfree_rate1,
                    0,
                    time_to_expiration,
                    option_strike,
                    leg_iv,
                ...
                leg_payoff = (
                    1 * leg_payoff
                    if config_leg_obj.action.upper() == "BUY"
                    else -1 * leg_payoff
                )
        ...
        if option_type == "CALL":
            if buy_or_sell == "BUY":
                ...
                if underlying_price_expiry >= option_strike:
                    payoff += (
                        quantity
                        * (underlying_price_expiry - option_strike)
                        * combination_multiplier
                    )
                else:
                    payoff += 0
    ...
    @staticmethod
    def calculate_combination_premium(
        list_of_legs_tuple,
        list_of_config_leg_object,
    ):
        # Default Values
        ...
        for leg, config_leg_obj in zip(list_of_legs_tuple, list_of_config_leg_object):
            ...
            if (
                ...
                continue
            instrument_object_for_leg_prem = copy.deepcopy(
                strategy_variables.map_instrument_id_to_instrument_object[instrument_id]
            )
