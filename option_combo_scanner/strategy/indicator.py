...
class Indicator:
    def __init__(self, data):
        ...
        self.average_underlying_hv_over_n_days = data.get(
            "average_underlying_hv_over_n_days", None
        )
        self.absoulte_change_in_underlying_over_n_days = data.get(
            "absoulte_change_in_underlying_over_n_days", None
        )
        self.absoulte_change_in_underlying_over_one_day = data.get(
            "absoulte_change_in_underlying_over_one_day", None
        )
        self.percentage_change_in_underlying_over_n_days = data.get(
        ...
        self.absolute_change_in_avg_iv_since_yesterday = data.get(
            "absolute_change_in_avg_iv_since_yesterday", None
        )
        self.percentage_change_in_avg_iv_since_yesterday = data.get(
        ...
        self.percentage_change_in_rr_since_yesterday_d1 = data.get(
            "percentage_change_in_rr_since_yesterday_d1", None
        )
        self.percentage_change_in_rr_since_yesterday_d2 = data.get(
            "percentage_change_in_rr_since_yesterday_d2", None
        )
        self.percentage_change_in_rr_since_14_day_d1 = data.get(
            "percentage_change_in_rr_since_14_day_d1", None
        )
        self.percentage_change_in_rr_since_14_day_d2 = data.get(
        ...
        self.chg_in_call_opt_price_since_yesterday_d1 = data.get(
            "chg_in_call_opt_price_since_yesterday_d1", None
        )
        self.chg_in_call_opt_price_since_yesterday_d2 = data.get(
            "chg_in_call_opt_price_since_yesterday_d2", None
        )
        self.chg_in_put_opt_price_since_yesterday_d1 = data.get(
            "chg_in_put_opt_price_since_yesterday_d1", None
        )
        self.chg_in_put_opt_price_since_yesterday_d2 = data.get(
            "chg_in_put_opt_price_since_yesterday_d2", None
        )

        self.chg_in_call_opt_price_since_nth_day_d1 = data.get(
            "chg_in_call_opt_price_since_nth_day_d1", None
        )
        self.chg_in_call_opt_price_since_nth_day_d2 = data.get(
            "chg_in_call_opt_price_since_nth_day_d2", None
        )
        self.chg_in_put_opt_price_since_nth_day_d1 = data.get(
            "chg_in_put_opt_price_since_nth_day_d1", None
        )
        self.chg_in_put_opt_price_since_nth_day_d2 = data.get(
            "chg_in_put_opt_price_since_nth_day_d2", None
        )

        self.put_call_volume_ratio_current_day = data.get(
            "put_call_volume_ratio_current_day", None
        )
        self.put_call_volume_ratio_average_over_n_days = data.get(
            "put_call_volume_ratio_average_over_n_days", None
        )
        self.absolute_pc_change_since_yesterday = data.get(
    ...
    def map_indicator_id_to_indicator_object(self):
        ...
        strategy_variables.map_indicator_id_to_indicator_object[self.indicator_id] = (
            self
        )

        # Used by the Impact Calculation
        strategy_variables.map_instrument_to_indicator_id[self.instrument_id] = (
            self.indicator_id
        )
        ...
        if self.underlying_conid not in strategy_variables.map_con_id_to_contract:
            ...
            strategy_variables.map_con_id_to_contract[self.underlying_conid] = (
                underlying_contract
            )

    def get_tuple(self):
        # LastAllowedTime: CurrentTime - CacheTime, Permissible time for the indicator cache
        indicator_cache_time_in_seconds: int = (
            strategy_variables.indicator_cache_time_in_seconds
        )
        current_time = datetime.datetime.now(variables.target_timezone_obj)
        last_allowed_time = current_time - datetime.timedelta(
            seconds=indicator_cache_time_in_seconds
        )

        # Decode the values
        current_underlying_hv_value = self.decode_value(
            self.current_underlying_hv_value, last_allowed_time
        )
        ...
        # Decode the values
        absolute_pc_change_since_yesterday = self.decode_value(
            self.absolute_pc_change_since_yesterday, last_allowed_time
        )
        absolute_change_in_avg_iv_since_yesterday = self.decode_value(
            self.absolute_change_in_avg_iv_since_yesterday, last_allowed_time
        )
        ...
        absoulte_change_in_underlying_over_n_days = self.decode_value(
            self.absoulte_change_in_underlying_over_n_days, last_allowed_time
        )
        absoulte_change_in_underlying_over_one_day = self.decode_value(
            self.absoulte_change_in_underlying_over_one_day, last_allowed_time
        )
        ...
        for val_ in [
            self.decode_value(
                self.chg_in_call_opt_price_since_yesterday_d1, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_call_opt_price_since_yesterday_d2, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_put_opt_price_since_yesterday_d1, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_put_opt_price_since_yesterday_d2, last_allowed_time
            ),
        ]:
        ...
        for val_ in [
            self.decode_value(
                self.chg_in_call_opt_price_since_nth_day_d1, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_call_opt_price_since_nth_day_d2, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_put_opt_price_since_nth_day_d1, last_allowed_time
            ),
            self.decode_value(
                self.chg_in_put_opt_price_since_nth_day_d2, last_allowed_time
            ),
        ]:
        ...
        if strategy_variables.flag_test_print:
            print(
                f"Indicator ID: {self.indicator_id}, Chg Underlying/Chg Opt Price: {res}"
            )
        ...
        tup = [
            ...
            self.decode_value(
                self.average_underlying_hv_over_n_days, last_allowed_time
            ),
            self.decode_value(
                self.absoulte_change_in_underlying_over_n_days, last_allowed_time
            ),
            self.decode_value(
                self.percentage_change_in_underlying_over_n_days, last_allowed_time
            ),
            current_hv_minus_iv,
            ...
            self.decode_value(
                self.percentage_change_in_avg_iv_since_yesterday, last_allowed_time
            ),
            ...
            self.decode_value(
                self.percentage_change_in_rr_since_yesterday_d1, last_allowed_time
            ),
            self.decode_value(
                self.percentage_change_in_rr_since_yesterday_d2, last_allowed_time
            ),
            self.decode_value(
                self.percentage_change_in_rr_since_14_day_d1, last_allowed_time
            ),
            self.decode_value(
                self.percentage_change_in_rr_since_14_day_d2, last_allowed_time
            ),
            ...
            self.decode_value(
                self.put_call_volume_ratio_current_day, last_allowed_time
            ),
            self.decode_value(
                self.put_call_volume_ratio_average_over_n_days, last_allowed_time
            ),
            absolute_pc_change_since_yesterday,
            # Computed
            pc_change_iv_change,
